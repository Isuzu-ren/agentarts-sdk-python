...
def repo_root() -> str:
    ...
    return str(Path(plugins_root()).parent)


def plugins_root() -> str:
    """Return the absolute path to ``agentarts-memory-plugins/``."""
    p = Path(installer_root())
    while p.parent != p:
        if p.name == "agentarts-memory-plugins":
            return str(p)
        p = p.parent
    return str(p)
...
def hermes_source() -> str:
    ...
    return str(Path(plugins_root()) / "agentarts-memory-hermes")


def code_agent_source() -> str:
    """Return the code_agent plugin source directory."""
    return str(Path(plugins_root()) / "agentarts-memory-code_agent")
...
def code_agent_scripts() -> list[str]:
    ...
    scripts_dir = str(Path(code_agent_source()) / "scripts")
    return [str(Path(scripts_dir) / f) for f in CODE_AGENT_SCRIPTS]


def claude_hooks_template() -> str:
    """Return the path to hooks/hooks.json (Claude template, 12 hooks)."""
    return str(Path(code_agent_source()) / "hooks" / "hooks.json")


def codex_hooks_template() -> str:
    """Return the path to hooks/hooks.codex.json (Codex template, 6 hooks)."""
    return str(Path(code_agent_source()) / "hooks" / "hooks.codex.json")
...
def opencode_files() -> dict[str, str]:
    ...
    """
    oc_dir = str(Path(code_agent_source()) / "opencode")
    return {
