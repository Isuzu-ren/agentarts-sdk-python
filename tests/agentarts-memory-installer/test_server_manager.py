...
import agentarts_memory_installer.server_manager as sm
from agentarts_memory_installer.server_manager import start, status, stop
...
class TestStart:
    ...
    def test_not_installed_installs_then_starts(self, monkeypatch, tmp_path):
        _set_home(monkeypatch, tmp_path)
        monkeypatch.setattr(sm, "_is_running", lambda: False)
        monkeypatch.setattr(sm, "_is_installed", lambda: False)
        monkeypatch.setattr(sm, "confirm", lambda prompt, default=True: True)
        monkeypatch.setattr(sm, "_install_server", lambda: True)
    ...
    def test_install_fails(self, monkeypatch, tmp_path):
        _set_home(monkeypatch, tmp_path)
        monkeypatch.setattr(sm, "_is_running", lambda: False)
        monkeypatch.setattr(sm, "_is_installed", lambda: False)
        monkeypatch.setattr(sm, "confirm", lambda prompt, default=True: True)
        monkeypatch.setattr(sm, "_install_server", lambda: False)

        assert start() == 1

    def test_install_declined(self, monkeypatch, tmp_path):
        _set_home(monkeypatch, tmp_path)
        monkeypatch.setattr(sm, "_is_running", lambda: False)
        monkeypatch.setattr(sm, "_is_installed", lambda: False)
        monkeypatch.setattr(sm, "confirm", lambda prompt, default=True: False)
    ...
    def test_process_dies_immediately(self, monkeypatch, tmp_path):
        _set_home(monkeypatch, tmp_path)
        monkeypatch.setattr(sm, "_is_running", lambda: False)
        monkeypatch.setattr(sm, "_is_installed", lambda: True)
        monkeypatch.setattr(sm.time, "sleep", lambda x: None)
